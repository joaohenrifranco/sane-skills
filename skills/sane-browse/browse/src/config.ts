 ...
 *   2. git rev-parse --show-toplevel → projectDir/.agents/.context/
...
export function resolveConfig(
  ...
  if (env.BROWSE_STATE_FILE) {
    ...
    projectDir = path.resolve(stateDir, '../..'); // parent of .agents/.context/
  } else {
    projectDir = getGitRoot() || process.cwd();
    stateDir = path.join(projectDir, '.agents/.context');
...
/**
 * Create the .agents/.context/ state directory if it doesn't exist.
 * Throws with a clear message on permission errors.
 *
 * .gitignore is managed manually (wildcard + .gitkeep exception).
 */
